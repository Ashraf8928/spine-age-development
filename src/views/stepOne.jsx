...
export default function StepOne({ validationErrors = [] }) {
    const { selections, setAnswer } = useSpineForm();
    const questions = questionAnswer[0].step_one;
    const questionRefs = useRef({});

    const handleAnswer = (questionId, answerId, point) => {
        setAnswer('step_one', questionId, answerId, point);
    };
    ...
    return (
        <div style={{ padding: '24px', maxHeight: '400px', overflowY: 'auto' }}>
            ...
            {questions.map((q) => {
                const selectedAnswerId = selections.step_one[q.id]?.answerId;
