...
export default function StepSix({ validationErrors = [] }) {
    const { selections, setAnswer } = useSpineForm();
    const questions = questionAnswer[5].step_six;
    const questionRefs = useRef({});

    const handleAnswer = (questionId, answerId, point) => {
        setAnswer('step_six', questionId, answerId, point);
    };
    ...
    return (
        <div style={{ padding: '24px', maxHeight: '400px', overflowY: 'auto' }}>
            ...
            {questions.map((q) => {
                const selectedAnswerId = selections.step_six[q.id]?.answerId;
