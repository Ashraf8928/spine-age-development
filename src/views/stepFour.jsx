...
export default function StepFour({ validationErrors = [] }) {
    const { selections, setAnswer } = useSpineForm();
    const questions = questionAnswer[3].step_four;
    const questionRefs = useRef({});

    const handleAnswer = (questionId, answerId, point) => {
        setAnswer('step_four', questionId, answerId, point);
    };
    ...
    return (
        <div style={{ padding: '24px', maxHeight: '400px', overflowY: 'auto' }}>
            ...
            {questions.map((q) => {
                const selectedAnswerId = selections.step_four[q.id]?.answerId;
