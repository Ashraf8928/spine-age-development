...
export default function StepTwo({ validationErrors = [] }) {
    const { selections, setAnswer } = useSpineForm();
    const questions = questionAnswer[1].step_two;
    const questionRefs = useRef({});

    const handleAnswer = (questionId, answerId, point) => {
        setAnswer('step_two', questionId, answerId, point);
    };
    ...
    return (
        <div style={{ padding: '24px', maxHeight: '400px', overflowY: 'auto' }}>
            ...
            {questions.map((q) => {
                const selectedAnswerId = selections.step_two[q.id]?.answerId;
